...
def getNextNode(pheromoneMatrix, visibilityMatrix, travelTimeMatrix, tabuList, alpha, beta, Pubs, Ant):
    ...
    if len(tabuList) < 18:
        ...
        for i in range(len(probabilities)):
            ...
            if i not in tabuList:
                ...
                # # am in Lindholmen, stay in Lindholmen
                # if (currentPosition == 4 or i == 4) and (currentPosition == 17 or i == 17):
                #     ratio = 83.3
                # # am in Lindholmen, go to Chalmers vice versa
                # elif (currentPosition == 4 or i == 4 or currentPosition == 17 or i == 17):
                #     ratio = 12
                # # am in Chalmers, stay in Chalmers
                # else:
                #     ratio = 83.3
                ...
                tmp2 = (1/(travelPart + waitingTimePart)) ** beta
                tmp3 = tmp1 * tmp2
    ...
    # This is to consider the relocation of lindholmen pubs.
    # if (Pubs[currentPosition].pubID == 4 or Pubs[node].pubID == 4) and (Pubs[currentPosition].pubID == 17 or Pubs[node].pubID == 17):
    #     travelTime = (getDistance(Pubs[currentPosition], Pubs[node])) / 83.3
    # elif Pubs[currentPosition].pubID == 4 or Pubs[node].pubID == 4 or Pubs[currentPosition].pubID == 17 or Pubs[node].pubID == 17:
    #     travelTime = (getDistance(Pubs[currentPosition], Pubs[node])) / 12
    # else:
    #     travelTime = (getDistance(Pubs[currentPosition], Pubs[node])) / 83.3
