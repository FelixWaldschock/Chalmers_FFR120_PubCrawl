...
def initPubs(filePath):
    ...
    for i in range(pubsList.shape[0]):
        ...
        sigma = pubsList[i][8]
        peakTimeOpening = pubsList[i][9]
        sigmaOpeningTime = pubsList[i][10]
        openingPopularity = pubsList[i][11]
        # create the Pub
        pub = Pub.Pub(pubID, pubName, openingTime, closingTime, popularity, posX, posY, peakTime, sigma, peakTimeOpening, sigmaOpeningTime, openingPopularity)
        Pubs.append(pub)
