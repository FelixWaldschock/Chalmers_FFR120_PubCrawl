    ...
    for i in range(popSize):
        ...
        if pathLength < minimumPathLength:
            ...
            if Plotting:
                ...
                # fig.suptitle('Travel path, depending on traveltime and closing time \nPub Crawl, iteration: ' + str(iter*(popSize)) + ', path length: ' + str(int(minimumPathLength)))
                fig.suptitle(f'Travel path, depending on traveltime and closing time \nPub Crawl, iteration:  {iter * popSize + (i + 1)}, path length: {minimumPathLength:.3f}', y=0.98)
...
axs[2].text(0.95, 0.25, r'$\textbf{Population size}$: ' + str(popSize) +
            ...
            usetex=True)
if Plotting:
    plt.savefig('ClosingTime_PubCrawl.svg', format='svg')
    print("Figure saved")
